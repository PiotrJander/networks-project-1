...
int main()
{
    ...
    do {
        ...
        // tzn nie akceptujemy więcej połączeń
        if (finish == TRUE && client[0].fd >= 0) {
            if (close(client[0].fd) < 0)
                perror("close");
            client[0].fd = -1;  // wartość -1 oznacza, że poll ignoruje
        }
        ...
        if (ret < 0)
            ...
            // events occurred in some sockets

            if (finish == FALSE && (client[0].revents & POLLIN)) {

                // in each loop we can accept one new connection
                msgsock =
                ...
                if (msgsock == -1)
                    ...
                    // in this loop we create the poll array entry for the new connection
